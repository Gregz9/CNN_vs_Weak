...
% Given the complexity of the network needed to achieve good results when using a CNN, we do not expect much when combining PCA, which is a much less sophisticated form of feature extraction than convolutional layers in a CNN, with a traditional neural network. Regardless, using a PCA-NN model, we extracted a varying amount of principal components and used these as features in a neural network composed of three dense layers. Using hyperparameter tuning, we found that a number of nodes in each layer equal to $1000$, a lambda of $0.001$ and a learning rate of $0.001$ gave the best result. However, even when using the best hyperparameters, we only achieved a test accuracy of $80.0\%$, which is substantially lower than Kermany et Al's $92.8\%$ and our own CNN model's $88.9\%$. This result shows why convolutional layers have become the de facto standard for image recognition, being able to extract features with much higher precision than simply calculating the principal components of a dataset. By the use of gradient descent, CNNs are able to tune the convolutional kernels to have the values which best fit the dataset, and as such they are able to be trained to solve many different problems. PCA, on the other hand, cannot be tuned or trained to better extract the relevant features, the only parameter than be changed is the amount of principal components to keep. With these poor results, no justification based on computational savings can be made, especially when CNNs can be trained extremely quickly on modern GPUs.
